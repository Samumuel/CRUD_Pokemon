...
<table class="table table-striped p-3 rounded-4 border border-5 border-primary bg-light">
    ...
    <?php foreach($lista as $pokemon): ?>
        <tr>
            <td><?= $pokemon->getId() ?></td>
            <td><img src="<?= $pokemon->getLink() ?>" alt="Foto" width="100px" height="100px"> </td>
            ...
            <td>
                <button onclick="carregarGeracao(<?= $pokemon->getGeracao()->getId() ?>)" >Ver Geração Ajax</button>
            </td>
        </tr>
    <?php endforeach; ?>
</table>

<table hidden id="tbGeracao" class="table table-striped p-3 rounded-4 border border-5 border-primary bg-light" >
    <!-- Cabeçalho -->
    <tr>
        <th>ID</th>
        <th>Numero</th>
