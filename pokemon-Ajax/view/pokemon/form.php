...
foreach($geracaos as $gr){
    array_push($geracaoAni, $gr->getAnime()); 
}
//print_r($geracao);

$tipo1 = $pokemon ? $pokemon->getTipo1() : "";
$tipo2 = $pokemon ? $pokemon->getTipo2() : "";
$geracao = $geracao ? $geracao->getAnime() : "";
    ...
    <div class="col-md-6 col-lg-4">
        <main class="form-signin w-100 m-auto text-center">
            ...
            <form method="POST" action="" border="1" class="p-3 rounded-4 border border-5 border-primary bg-light">
                ...
                <div class="mb-3">
                    <select id="selTipo1" class="form-select" name="tipo1">
                ...
                <div class="mb-3 border-primary">
                    <select id="selTipo2" class="form-select" name="tipo2">
                        <option value="">Selecione o Tipo</option>
                            <?php foreach($tipos as $tp): ?>
                ...
                <div class="mb-3 border-primary">
                    <input type="number" id="numEvolucao" name="evolucao"
                        ...
                        value="<?= $pokemon ? $pokemon->getEvolucao() : '' ?>" min="1" max="3">
                </div>

                <div class="mb-3 border-primary">
                    <select id="selGera" class="form-select" name="geracao">
                        <option value="">Selecione a Geração</option>
                            <?php foreach($geracaoAni as $gr): ?>
                                <option value="<?= $gr?>" <?php if ($geracao == "$tp") {
                                    echo "selected";
                                }; ?>><?php echo $gr; ?></option>
                        <?php endforeach; ?>
                    </select>
